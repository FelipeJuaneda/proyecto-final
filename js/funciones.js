...
window.addEventListener("scroll", function () {
  ...
  tituloEfectoScroll.style.marginTop = scrollenY * 0.3 + "px";
  subtituloEfectoScroll.style.marginTop = scrollenY * 0.5 + "px";
});

//TITULO GALERIA COMERCIAL
const typed = new Typed(".typed", {
  strings: ["<i>Comercial</i>", "<i>Artesanal</i>", "<i>Colonial</i>"],
...
//--------------------------------FUNCION GENERADORA DE INTERFAZ CON BOTON "LOCALES"---------------------------
let botonDispo = document.getElementById("botonLocaldispo");
botonDispo.addEventListener("click", localesBoton);
function localesBoton() {
  ...
  inputFiltro.addEventListener("input", function () {
    ...
    if (inputFiltro.value != "" && inputFiltro.value <= 10) {
      ...
      const filtrados = local.find(
        (locales) => locales.id == inputFiltro.value
      );
    ...
    //por si no se encuentra local
      ...
      localFiltradoCont.innerHTML =
        '<p class="text-center fs-2" style="color: red">No se encontro Local - Verifique numero</p>';
    }
  });
  ...
  let maxDeScroll =
    localessionoClase.scrollWidth - localessionoClase.clientWidth;
  let intervalo = null;
  let velocidadCarrusel = 1;
  //funcion de carrusel
  const empezarCarrusel = () => {
    intervalo = setInterval(() => {
      localessionoClase.scrollLeft =
        localessionoClase.scrollLeft + velocidadCarrusel;
...
function obtenerLocales() {
  "ListaLocales" in localStorage &&
    local.localStorage.getItem("ListaLocales").split(",");
}
...
setTimeout(() => {
  Swal.fire({
    title:
      '<strong>Seguinos en las <u style="color:var(--color-naranja)">Redes</u></strong>',
    icon: "info",
    html:
      "En <b>Instagram </b> " +
  ...
  for (const producto of lista) {
    ...
    prod.innerHTML = `
        <div class"contenedorModalCarrito w-100 m-auto">
            <h2 style="color:var(--color-naranja);">${
              producto.nombre
            } </h2>        
            <span>Precio: $ ${producto.precio} /</span>
            <span style="color:var(--color-negro);">Cantidad: ${
              producto.cantidad
            } /</span>
...
function addCarrito() {
  ...
  this.parentNode.children[3].innerHTML =
    "Subtotal: " + agregarProducto.subTotal();
  localStorage.setItem("Carrito", JSON.stringify(carrito));
